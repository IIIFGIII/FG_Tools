bl_info = {
	...
	"warning": "Tested/guaranteed work on official relese 2.83.13LTS",
	...
	def execute(self, context):
		...
		if btu_p.btu_exp_path != '//Export/':
			...
			if not os.path.exists(exp_folder):
				self.report({'ERROR'}, f' Your custom path {btu_p.btu_exp_path} not exist! Trying default "//Export/" path.')		
...
class BTU_Settings_Props(bpy.types.PropertyGroup):
	...
	btu_exp_scale: bpy.props.FloatProperty(
		description='Will export models in this scale factor', 
		default=1.0, 
		min=0.001, 
		max=1000.0, 
		step=0.01)
	...
	btu_exp_replacedot: bpy.props.BoolProperty(
		name='',
		description='Replace " . " (dot) in object name for FBX files name',
		default=False)
	...
	btu_copy_replacedot: bpy.props.BoolProperty(
		name='',
		description='Replace " . " (dot) in object name copied for ue UE',
		default=False)
